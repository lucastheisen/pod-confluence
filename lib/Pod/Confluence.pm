...
sub _confluence_macro_end {
    ...
    my @optional_elements = (
        ($options{has_text} ? ']]></ac:plain-text-body>' : ()),
    );
...
sub _init {
    ...
    $self->{confluence_schema_version} = 
        $options{confluence_schema_version} || 1;
...
sub start_Verbatim {
    my ($self, $attribute_hash) = @_;
    $self->_print($self->_confluence_macro_start('code', has_text => 1,
        parameters => {
            language => 'perl'
        }));
    $self->{in_cdata} = 1;
